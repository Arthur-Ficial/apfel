...
- Version: `1.12.0` (source of truth: `.version`)
- Tests: 1292 unit + 549 integration
...
swift run apfel-tests          # unit tests only (1292 tests)
...
`make test` builds the release binary, runs all 1292 unit tests, starts test servers, runs all 549 integration tests (two phases, #374: the model-free partition in parallel via pytest-xdist, then the serial model phase), and cleans up. This is the single command for development.
...
| Tests | `Tests/apfelTests/` (1292 unit), `Tests/integration/` (549 integration) |
