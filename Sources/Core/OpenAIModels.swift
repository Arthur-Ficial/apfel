...
public struct ChatCompletionRequest: Decodable, Sendable, Equatable, Hashable {
    ...
    /// Backward-compatible overload preserving the v1.11.0 designated init
    /// (with `max_completion_tokens` but without `parallel_tool_calls`).
    public init(
        model: String,
        messages: [OpenAIMessage],
        stream: Bool? = nil,
    ...
    /// Backward-compatible overload preserving the pre-v1.11.0 init
    /// (without `max_completion_tokens` or `parallel_tool_calls`).
    public init(
        model: String,
        messages: [OpenAIMessage],
        stream: Bool? = nil,
