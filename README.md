...
Every Mac with Apple Silicon has a **built-in LLM** - Apple's on-device foundation model, shipped as part of Apple Intelligence. Apple provides the [https://developer.apple.com/documentation/foundationmodels](https://developer.apple.com/documentation/foundationmodels) framework (macOS 26+) to access it, but only exposes it through Siri and system features. **apfel wraps it** in a CLI and an HTTP server - so you can actually use it.
...
- **GUI apps** - chat client [https://apfel-chat.franzai.com](https://apfel-chat.franzai.com), clipboard actions [https://apfel-clip.franzai.com](https://apfel-clip.franzai.com), debug inspector [https://github.com/Arthur-Ficial/apfel-gui](https://github.com/Arthur-Ficial/apfel-gui)

![apfel CLI](screenshots/cli.png)

## Requirements & Install

- **macOS 26 Tahoe or newer**, Apple Silicon (M1+), [Apple Intelligence enabled](https://support.apple.com/en-us/121115)
...
Shell note: if your prompt contains `!`, prefer single quotes in `zsh`/`bash` so history expansion does not break copy-paste.
...
Native SwiftUI debug inspector with request timeline, MCP protocol viewer, chat, TTS/STT - all on-device. **[https://github.com/Arthur-Ficial/apfel-gui](https://github.com/Arthur-Ficial/apfel-gui)**

## MCP Tool Support

Attach [https://modelcontextprotocol.io/](https://modelcontextprotocol.io/) tool servers with `--mcp`. apfel discovers tools, executes them automatically, and returns the final answer. No glue code needed.
...
Full API spec: [https://github.com/openai/openai-openapi](https://github.com/openai/openai-openapi)
...
apfel --temperature 0.0 --seed 42 --max-tokens 50 "Explain quantum computing"

# --permissive
apfel --permissive -f long-document.md "Summarize this"

# --retry, --debug, --stream
apfel --retry 5 --debug --stream "Write a haiku about code"

# --chat, --context-strategy, --context-max-turns, --context-output-reserve
apfel --chat --context-strategy sliding-window --context-max-turns 6 --context-output-reserve 256

# --serve, --port, --host
apfel --serve --port 3000 --host 0.0.0.0

# --cors, --allowed-origins
apfel --serve --cors --allowed-origins "https://myapp.com,https://staging.myapp.com"

# --token, --token-auto, --public-health
apfel --serve --token "my-secret-token"
apfel --serve --token-auto --host 0.0.0.0 --public-health

# --no-origin-check, --footgun, --max-concurrent
apfel --serve --no-origin-check --max-concurrent 2
...
- [https://github.com/Arthur-Ficial/apfel-chat](https://github.com/Arthur-Ficial/apfel-chat) - Multi-conversation macOS chat client. Private, on-device, no API keys. Streaming markdown, speech I/O, image analysis. Install: `brew install Arthur-Ficial/tap/apfel-chat`
- [https://github.com/Arthur-Ficial/apfel-clip](https://github.com/Arthur-Ficial/apfel-clip) - AI clipboard actions from the menu bar
- [https://github.com/Arthur-Ficial/apfel-gui](https://github.com/Arthur-Ficial/apfel-gui) - Native macOS debug GUI (inspector, MCP viewer, TTS/STT)
- [https://github.com/Arthur-Ficial/apfel-mcp](https://github.com/Arthur-Ficial/apfel-mcp) - Three token-budget-optimized MCP servers (url-fetch, ddg-search, compound search+fetch) tuned for apfel's 4096-token context window. Install: `brew install Arthur-Ficial/tap/apfel-mcp`

## Community Projects

Things the community has built on top of apfel. Built something? Open an issue and we will add it.

- [https://apfelclaw.yamanlabs.com/](https://apfelclaw.yamanlabs.com/) by [https://github.com/julianYaman](https://github.com/julianYaman) - Local AI agent that reads files, calendar, mail, and Mac status via read-only tools. Swift + Vapor backend, Bun TUI, optional Telegram remote. Repo: [https://github.com/julianyaman/apfelclaw](https://github.com/julianyaman/apfelclaw)
