...
- Numeric and array bounds reach the model. `minimum` / `maximum` / `exclusiveMinimum` / `exclusiveMaximum` on `integer` and `number` nodes and `minItems` / `maxItems` on arrays were parsed and then discarded; they now compile to FoundationModels generation guides and array element limits, so a `rating` declared `1..5` is generated in `1..5` and a `tags` array declared `minItems: 2, maxItems: 3` has two or three entries. Exclusive bounds become the equivalent inclusive bound (the adjacent integer, or the adjacent representable `Double`), and the draft-4 boolean `exclusiveMinimum: true` form is honoured. Supported and rejected keywords are documented in `docs/openai-api-compatibility.md`, "JSON Schema support" (#479).

### Fixed

- Two differently shaped nested objects (or string enums) that share a property name no longer lose one shape. FoundationModels hoists every named node into `$defs` by name and lets the first definition win, and apfel named nested nodes after their property key - so with `person.contact {email}` and `company.contact {phone}` in one schema, `company.contact` was generated with the email shape: HTTP 200, `strict: true` echoed back, and output that violated the caller's schema with nothing in the response to show it. Object and enum names are now unique per distinct shape within one schema document; a later, differently shaped node with the same key is titled `contact_2`. Identical shapes still share one name (#479).
- Validation keywords that on-device schema-guided generation cannot enforce are rejected before generation instead of being silently dropped. The response is a 400 `invalid_request_error` naming the keyword and its JSON pointer (`"multipleOf" at #/properties/price ...`) on `/v1/chat/completions` - also with `stream: true`, before any SSE frame - and `/v1/responses`; `apfel --schema` exits 2 with the same message and the file name. Rejected: `multipleOf`; `minLength`, `maxLength`, `pattern`, `contentEncoding`, `contentMediaType`, `contentSchema`; `uniqueItems: true`, `contains`, `minContains`, `maxContains`, `prefixItems`, `additionalItems`; `minProperties`, `maxProperties`, `patternProperties`, `propertyNames`, `dependentRequired`, `dependentSchemas`, `dependencies`, schema-valued `additionalProperties` / `unevaluatedProperties` / `unevaluatedItems`; `enum` / `const` on non-string types; `not`, `if`, `then`, `else`, `$dynamicRef`, `$recursiveRef`. Unsatisfiable schemas are rejected the same way: `minimum` above `maximum`, `minItems` above `maxItems`, negative or fractional counts, an empty `enum`, `required` naming a property that is not declared. Keywords that do not apply to the node's type (`minLength` on an integer), unknown keywords, boolean `additionalProperties`, and annotations (`title`, `default`, `examples`, `format`, `$schema`, `$id`, `nullable`, `x-*`) stay accepted. Tool parameter schemas keep the documented text-injection fallback rather than failing the request. A string `enum` without an explicit `type` (Pydantic `Literal[...]`) now parses as a string choice instead of an empty object, and a string `const` as a single-value choice (#479).
...
- Trimming client-supplied history no longer splits a tool exchange. Every strategy selected individual transcript entries (`suffix(k)` / `prefix(k)` / a per-entry window), so a boundary landing between an assistant `tool_calls` message and the `tool` messages answering it handed the model an orphaned call or an unexplained result, and a trailing tool result could be trimmed away entirely while the synthetic "respond to the tool result above" prompt still referenced it - under `oldest-first` the continuation lost the results even when every message was small. A pure `ApfelCore.ToolExchangeGrouping` now treats a tool-call message and all its matching results as one indivisible group; `newest-first`, `oldest-first`, `sliding-window` and `summarize` select whole groups (a tool exchange counts as one `x_context_max_turns` / `--context-max-turns` turn), the exchange a trailing tool result belongs to is pinned and priced as required (too large for the runtime-derived budget is a context-overflow error before generation, not a silent drop), and the summarizer describes tool calls and results instead of dropping them. Tool results resolve their name through `tool_call_id` instead of the optional client `name`, and a result that answers no call, a call with a missing or duplicate result, or a `tool` message without `tool_call_id` is rejected up front with the offending `messages[i]` path - 400 on `/v1/chat/completions`, exit 2 for `apfel --messages` - matching the OpenAI API. The `--mcp` auto-execute follow-up now prices a large tool result against the transcript it will actually build (instructions, originating prompt, tool-call entry, final prompt) instead of the joined message text, so the #221 truncation leaves room for the pinned exchange (#482).
- `tool_choice` and `parallel_tool_calls` are now enforced at the response boundary instead of only steered by prompt instructions. Before, `tool_choice: "none"` still let tool-call-shaped model output through as an executable `tool_calls` response (and, with `--mcp`, still auto-executed it), a forced choice the model ignored came back as HTTP 200 with a plain-text answer that looked complete, a call to a function the client never defined was exposed as-is, and `parallel_tool_calls` was not decoded at all. A pure `ApfelCore.ToolPolicy` now resolves the contract from `tool_choice`, the effective tool set (client tools or attached MCP tools) and `parallel_tool_calls`, and judges every detected call on the non-streaming, streaming, MCP auto-execute and `/v1/responses` paths: `none` makes tool-shaped output ordinary content and disables MCP dispatch; a `required` or named choice that nothing in scope can satisfy is a 400 with `param: "tool_choice"` before generation; a named choice presents only that function to the model and may name an attached MCP tool; plain text under a forced choice, or a call to a function outside the scope, is a typed 500 (`code: "tool_choice_not_satisfied"` / `"tool_call_not_allowed"`, terminating an SSE stream with the same error object); `parallel_tool_calls: false` returns at most one call. `/v1/models` advertises `parallel_tool_calls` (#480).
...
- `apfel --model-info`, `/health` and `/v1/models` no longer present the assumed context-window floor as a measured value. `TokenCounter.contextSize` falls back to 4096 when `SystemLanguageModel.contextSize` reports 0, which it does for the whole macOS 27 cold start (~80s) and whenever the model is not ready - the floor is correct and stays, because without it `inputBudget` goes negative and the model never warms up (#192). What was wrong is that the fallback was rendered indistinguishably from a real reading. On macOS 26 that is invisible, since the floor equals the true window; on macOS 27 the true window is 8192, so every one of these surfaces reported half the machine's real capacity as fact, and `/health` is what an OpenAI-compatible client reads to size `max_tokens`. The measured/assumed distinction now lives in `ApfelCore.ContextWindowTracker`, `--model-info` marks an assumed value, and `/health` and `/v1/models` carry an additive `context_window_measured` boolean - `context_window` stays a positive integer, so the #192 guarantee and existing clients are unaffected (#491).
